        ...
        try:
            ...
            if pswd != confirm:
                ...
                with mysql.connector.connect(host="localhost", user="root", password="root",
                                             database="pythongroup") as sitedb:
                    cur = sitedb.cursor()
                    pswd_hash = generate_password_hash(pswd)  # hashing password

                    cur.execute("INSERT INTO Users (Username, FirstName, LastName, DOB, Pass) VALUES (%s,%s,%s,%s,%s)",
                                (usrnm, frst, last, dob, pswd_hash,))
                    sitedb.commit()
        ...
        try:
            ...
            if (type == "isbn"):
                ...
                e = wait.until(EC.visibility_of_element_located(
                    (By.XPATH, '//*[@id="bd-isbn"]/div/table/tbody/tr/td[1]/table/tbody/tr[2]/td[4]/div/span/a')))
                lowestNew = e.get_attribute('text')

                newLink = driver.find_element(By.XPATH,
                                              '//*[@id="bd-isbn"]/div/table/tbody/tr/td[1]/table/tbody/tr[2]/td[4]/div/span/a').get_attribute(
                    'href')
                ...
                cur.execute("INSERT IGNORE INTO Textbooks (Title, ISBN, Retail, Subj) VALUES (%s, %s, %s, %s)",
                            (title, query, lowestNew, sellerInfo,))
                ...
                e = wait.until(
                    EC.visibility_of_any_elements_located((By.XPATH, "//*[contains(text(), 'Click to pick a title')]")))
                clickTitle = driver.find_element(By.XPATH, '//*[@id="bd"]/ul/li/span/a').click()

                # find ISBN
                e = wait.until(EC.visibility_of_element_located(
                    (By.XPATH, '//*[@id="bd"]/table/tbody/tr/td[1]/table/tbody/tr[2]/td[3]/a')))
                isbn = e.get_attribute('innerHTML')

                e = wait.until(EC.visibility_of_element_located(
                    (By.XPATH, '//*[@id="bd"]/table/tbody/tr/td[1]/table/tbody/tr[2]/td[4]/div/span/a')))
                lowestNew = e.get_attribute('text')

                newLink = driver.find_element(By.XPATH,
                                              '//*[@id="bd"]/table/tbody/tr/td[1]/table/tbody/tr[2]/td[4]/div/span/a').get_attribute(
                    'href')
                sellerInfo = str(newLink)

                # insert into Textbooks if not found, update if found
                cur.execute("INSERT IGNORE INTO Textbooks (Title, ISBN, Retail, Subj) VALUES (%s, %s, %s, %s)",
                            (query, isbn, lowestNew.strip('$'), sellerInfo,))
...
def addListing():
    if request.method == 'POST':
        ...
        try:
            ...
            cur = sitedb.cursor()
            cur.execute("INSERT INTO Listings (Title, ISBN, Asking, HighestBid) VALUES (%s,%s,%s,%s)",
                        (title, isbn, askprc, 0,))
            sitedb.commit()
